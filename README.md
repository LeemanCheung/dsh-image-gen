...
`v0.4.1` is the current release (GPT Image 2.5 support plus the image-read recovery fix). For production, pin the exact commit you reviewed (`.../dsh-image-gen#<sha>`) rather than a moving branch; `main` carries unreleased work between tags.
...
- A card reads its image over the loopback RPC, and the Host authorizes that read from the session store, which is written asynchronously. A read can therefore arrive before the completed result is durable. `0.4.1` treats that case as recoverable rather than terminal: the Host keeps the just-generated reference for `servedResultTtlMs` (default 5 minutes; never a long-term authorization path), answers with a distinct `pending` reason, and the card retries with bounded backoff before showing an actionable message. Set `servedResultTtlMs: 0` to make the session store the sole authority at every moment.
