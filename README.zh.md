...
在 DeepSeek Harness 会话中使用 OpenAI `gpt-image-2.5-flare`（也可选 `gpt-image-2.5-sunburst` 与 `gpt-image-2`）生成图片；默认复用已登录的 Codex 订阅，也可显式切换为 API Key。
...
| 领域 | 已交付行为 |
| --- | --- |
| 工具与接入路径 | 暴露与 Codex 兼容的 `image_gen`，可选 `gpt-image-2.5-flare`、`gpt-image-2.5-sunburst` 与 `gpt-image-2`；默认复用可刷新的 `dsh-codex-connect` 订阅 OAuth，也可显式使用 DSH API Key 凭据。 |
...
- [GPT Image 2.5 Flare 模型](https://developers.openai.com/api/docs/models/gpt-image-2.5-flare)
...
dsh plugin --profile web add github:LeemanCheung/dsh-image-gen#v0.4.1
```

`v0.4.1` 是当前发布版本（GPT Image 2.5 支持 + 成图读取恢复修复）。生产环境请固定你已审查过的具体提交（`.../dsh-image-gen#<sha>`），不要跟随会移动的分支；`main` 在两个标签之间会包含未发布改动。
...
完成结果会分开记录请求和结果：`size` 来自最终图片字节的实际宽高；`requestedSize` / `requestedQuality` 保留调用参数；`qualitySource` 标明质量是 Provider 回报，还是仅为请求值回填；`model` 记录实际服务本次请求的模型。
...
2026-09-11 在 DSH `0.1.5-rc.1` 上的真实订阅请求确认：私有 Codex 端点接受 `gpt-image-2.5-flare` 与 `gpt-image-2.5-sunburst`，每次探针都返回 HTTP 200 并原样回传请求的模型名。端点并不按字面接受请求的 `size` 与 `quality`，而是自行决定交付尺寸与画质，但其画质预算跟随请求档位：`high` 返回 `1774x887`、1030 个图片输出 token，`max` 返回 `1536x1024`、1372 个 token。因此同一档位在订阅路径上的实际尺寸与消耗可能不同于 API Key 契约所记录的数值；结果中的 `size` / `quality` 始终描述 Provider 实际返回，`requestedSize` / `requestedQuality` 保留调用请求值。
...
Codex 订阅调用会消耗已登录 ChatGPT 套餐对应的图片生成额度。API Key 调用按所选模型、尺寸和质量计费；根据服务端指南，每张渐进局部图还会消耗额外图片输出 token。GPT Image 2.5 与 GPT Image 2 的 token 单价相同，但各画质档的图片输出 token 数不同——`xhigh` 与 `max` 高于 GPT Image 2 原先的 `high`。`partialImages` 不适用于订阅模式。
...
- ChatGPT Codex 订阅端点属于私有兼容接口。真实请求证实它接受两个 GPT Image 2.5 模型别名与 `xhigh` / `max` 档位，但会忽略字面的 `size` 与 `quality`，按自己的预算出图（2026-09-11 实测：`high` 为 `1774x887` / 1030 token，`max` 为 `1536x1024` / 1372 token）。官方仍未把它描述为 Image API 编辑端点，因此插件继续禁用订阅参考编辑和仅公开 API 支持的输出参数。
- 订阅路径的 `size` 与 `quality` 影响的是 Provider 自身预算，而非字面请求，因此同一档位交付的尺寸与消耗可能与 API Key 契约不同。
- 卡片通过回环 RPC 读取成图，而 Host 需要从会话存储中授权该次读取，会话存储是异步写入的。因此读取请求可能早于结果事件落盘。`0.4.1` 把这种情况当作可恢复而非终态：Host 会为刚生成的引用保留 `servedResultTtlMs`（默认 5 分钟，且绝不是长期授权通道），返回独立的 `pending` 原因，卡片按有界退避重试后才显示可操作的提示。把 `servedResultTtlMs` 设为 `0` 可让会话存储在任何时刻都是唯一授权来源。
...
- GPT Image 2.5 的 API Key 路径仅用确定性的 Provider 模拟响应验证，未在真实计费账号上调用。Codex 订阅路径已用真实订阅额度验证 `gpt-image-2.5-flare` 与 `gpt-image-2.5-sunburst`；真实 API Key 调用以及订阅端点上的 `gpt-image-2` 仍待验证。
