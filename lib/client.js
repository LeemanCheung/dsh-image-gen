window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function imageErrorDetails(value) {
			if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
			const details = value;
			if (details.reason !== "pending" && details.reason !== "inspection-failed" && details.reason !== "attachment-failed") return void 0;
...
`;
		...
		*/
		function imageErrorCopy(reason, message) {
			switch (reason) {
		...
		function ImageGenCard({ sessionId, callId, block, t, requestProgress, requestImage }) {
			...
			(0, react.useEffect)(() => {
				...
				(async () => {
					for (let attempt = 0;; attempt += 1) try {
						...
						const retryIn = error.reason === "pending" ? IMAGE_RETRY_MS[attempt] : void 0;
						if (retryIn === void 0) {
							setLoadError(error instanceof Error ? error.message : String(error));
							return;
						}
						await new Promise((resolve) => {
							timer = setTimeout(resolve, retryIn);
						});
						if (!live) return;
					}
				})();
			...
			const phase = progress?.state === "requesting" ? t("requesting") : progress?.state === "generating" ? t("rendering") : progress?.state === "saving" ? t("saving") : settled && finalImage !== void 0 ? t("ready") : settled && presentation !== void 0 && loadError === void 0 ? t("loading") : t("waiting");
			...
			const error = failed ? settled && block.isError ? resultError(block, t("noOutput")) : t("noOutput") : loadError ?? "";
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: failed ? result?.model ?? args.model ?? "GPT Image" : phase
								})]
							}),
		...
		function apply(ctx) {
			...
			const call = async (endpoint, payload, signal) => {
				...
				if (!result.ok) {
					...
					const error = new Error(imageErrorCopy(reason, result.error.message));
					if (reason !== void 0) error.reason = reason;
					throw error;
				}
				return result.value;
			};
