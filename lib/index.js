...
const MAX_AUTH_DOCUMENT_BYTES = 16384;
const REFRESH_WINDOW_MS = 3e5;
...
function activeProfileName(args = process.argv.slice(2)) {
	if (args[0] === "web") return "web";
	const index = args.indexOf("--profile");
	const name = index >= 0 ? args[index + 1] : void 0;
	return name !== void 0 && /^[a-zA-Z0-9_-]+$/.test(name) ? name : "web";
}
/** A linked plugin must resolve optional integrations from the active Profile too. */
async function loadInstalledCodexConnector(direct = () => import(CONNECTOR_PACKAGE), resolveInProfile = () => createRequire(join(dshHome(), "profiles", activeProfileName(), "package.json")).resolve(CONNECTOR_PACKAGE), loadResolved = (url) => import(url)) {
	try {
		return await direct();
	} catch (error) {
...
async function resolveCodexSubscriptionAuth(signal, loadConnector = loadInstalledCodexConnector, readDocument = defaultReadCodexAuthDocument, now = Date.now, trackWork) {
	signal.throwIfAborted();
	let loaded;
	try {
		...
		signal.throwIfAborted();
		if (credential === void 0) throw new Error("OpenAI Codex is signed out or its credential store is incompatible. Sign in again from Codex Connect settings.");
...
function isImageQuality(value) {
	return typeof value === "string" && IMAGE_QUALITIES.includes(value);
}
//#endregion
...
function imageModel(value) {
	const normalized = value.trim();
	if (normalized.length === 0 || normalized.length > MAX_MODEL_CHARS) throw new TypeError(`model must contain 1–${MAX_MODEL_CHARS} characters`);
	if (!/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u.test(normalized)) throw new TypeError("model must contain only letters, digits, and the characters . _ : -");
	return normalized;
}
...
function imageEditBody(options, request, reference) {
	...
	if (request.outputFormat !== "png" && request.outputCompression !== void 0) body.append("output_compression", String(request.outputCompression));
...
var OpenAIImageClient = class {
	...
	constructor(options) {
		...
		if (this.protocol === "codex-subscription") {
			if (baseUrl !== "https://chatgpt.com/backend-api/codex") throw new TypeError("Codex subscription credentials may be sent only to the first-party Codex endpoint");
	...
	async generate(request, signal, onProgress) {
		...
		if (this.protocol === "codex-subscription") {
			if (reference !== void 0) throw new ImageApiError("Reference-image generation requires API-key mode because the Codex subscription endpoint does not accept image edits.");
			if (request.outputFormat !== "png" || request.outputCompression !== void 0 || request.background === "transparent") throw new ImageApiError("This output option requires API-key mode and cannot be sent to the Codex subscription endpoint.");
		}
		const body = reference === void 0 ? JSON.stringify({
		...
		const endpoint = reference === void 0 ? this.generationEndpoint : this.editEndpoint;
		let lastError;
		for (let attempt = 1; attempt <= this.options.maxRetries + 1; attempt += 1) {
			...
			try {
				response = await this.fetchImpl(endpoint, {
					...
					redirect: "error",
					headers: {
						accept: reference === void 0 && this.protocol !== "codex-subscription" ? "text/event-stream" : "application/json",
						authorization: `Bearer ${this.options.apiKey}`,
						...reference === void 0 ? { "content-type": "application/json" } : {},
						...this.protocol === "codex-subscription" ? {
...
*/
const CODEX_SUBSCRIPTION_MODEL = "gpt-image-2.5-flare";
...
const Config = Schema.object({
	...
	defaultBackground: Schema.union([
		"auto",
		"opaque",
		"transparent"
	]).default("auto"),
	...
	servedResultTtlMs: Schema.number().min(0).max(6e5).step(1e3).default(3e5)
});
...
function normalizedReferenceImagePath(value) {
	if (value === void 0) return void 0;
	if (typeof value !== "string") throw new Error("reference_image_path must be a string");
	const normalized = value.trim();
	if (normalized.length === 0 || normalized.length > 4096) throw new Error("reference_image_path must contain 1–4096 characters");
	return normalized;
}
...
function referenceMediaType(filePath) {
...
async function referenceImageFromPath(ctx, exec, filePath) {
	const fs = ctx.get("fs");
	if (fs === void 0) throw new Error("reference_image_path requires a DSH filesystem provider");
	const mediaType = referenceMediaType(filePath);
	if (mediaType === void 0) throw new Error("reference_image_path must name a PNG, JPEG, or WebP image");
	...
	if (info === void 0 || info.type !== "file") throw new Error(`reference_image_path cannot read "${filePath}" as a regular file`);
...
function presentation(value) {
	if (!isRecord(value) || value.schema !== "dsh-image-gen/presentation-v1" || !isRecord(value.result)) return void 0;
	const result = value.result;
	if (result.schema !== "dsh-image-gen/result-v1" || typeof result.callId !== "string" || imageRef(result.image) === void 0) return void 0;
	return value;
}
function referenceValue(value) {
	return {
		...
		model: value.model,
		image: value.image,
		...value.referenceImage === void 0 ? {} : { referenceImage: value.referenceImage },
		size: value.size,
		quality: value.quality,
		...value.requestedSize === void 0 ? {} : { requestedSize: value.requestedSize },
		...value.requestedQuality === void 0 ? {} : { requestedQuality: value.requestedQuality },
		...value.providerSize === void 0 ? {} : { providerSize: value.providerSize },
		...value.qualitySource === void 0 ? {} : { qualitySource: value.qualitySource },
...
function referenceFromText(value) {
	...
	try {
		...
		if (!isRecord(parsed) || parsed.schema !== "dsh-image-gen/ref-v1" || typeof parsed.callId !== "string" || imageRef(parsed.image) === void 0) return void 0;
		return parsed;
	} catch {
		return;
	}
...
function apply(ctx, config) {
	...
	*/
	...
	const rememberServed = (sessionId, callId, value) => {
	...
	const recentlyServed = (sessionId, callId) => {
		if (!recallServed) return void 0;
		const key = keyOf(sessionId, callId);
	...
	const resolveImageAuth = async (signal, apiKeyReason) => {
		if (apiKeyReason !== void 0 && config.authMode === "codex-subscription") throw new Error(`${apiKeyReason} requires authMode auto with an API key fallback, or authMode api-key`);
		let codexError;
		if (apiKeyReason === void 0 && config.authMode !== "api-key") try {
	...
	ctx.on("tools/pre-execute", async (execution, next) => {
		...
		try {
			...
			return {
				kind: "deny",
				reason: error instanceof Error ? error.message : "reference_image_path is invalid"
			};
		}
		if (referencePath === void 0) return next();
		return {
			kind: "ask",
	...
	ctx.effect(() => ctx.connection.rpc.handle(IMAGE_GEN_RPC_CHANNEL, async (endpoint, payload, signal) => {
		...
		const ref = authorizedImage(inspection.events, callId) ?? recentlyServed(sessionId, callId);
		if (ref === void 0) return rpcError("pending", "The session store has not committed this image call yet. It becomes readable as soon as the completed result is durable.");
		try {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const requestedModel = args.model === void 0 ? void 0 : imageModel(args.model);
			...
			if (outputFormat === "png" && args.output_compression !== void 0) throw new Error("output_compression is supported only for JPEG and WebP");
			if (requestBackground === "transparent" && outputFormat === "jpeg") throw new Error("transparent backgrounds require PNG or WebP output");
			...
			try {
				const apiKeyReason = referenceImagePath !== void 0 ? "reference_image_path" : outputFormat !== "png" ? `${outputFormat.toUpperCase()} output` : requestBackground === "transparent" ? "transparent background output" : args.output_compression !== void 0 ? "output_compression" : void 0;
				const auth = await resolveImageAuth(requestSignal, apiKeyReason);
				requestSignal.throwIfAborted();
				const reference = referenceImagePath === void 0 ? void 0 : await referenceImageFromPath(ctx, exec, referenceImagePath);
				requestSignal.throwIfAborted();
				...
				const requestModel = requestedModel ?? (auth.kind === "codex-subscription" ? "gpt-image-2.5-flare" : config.model);
				const generated = await new OpenAIImageClient({
					...
					...reference === void 0 ? {} : { referenceImage: reference.input }
				...
				const saved = await ctx.attachments.saveImages([finalInput, ...reference === void 0 ? [] : [reference.save]]);
				const ref = saved[0];
				const referenceRef = saved[1];
				if (ref === void 0 || reference !== void 0 && referenceRef === void 0) throw new Error("The attachment service did not return every saved image reference.");
				requestSignal.throwIfAborted();
				const value = {
					...
					...referenceRef === void 0 ? {} : { referenceImage: imageRefValue(referenceRef) },
...
//#endregion
export { CODEX_SUBSCRIPTION_MODEL, Config, DEFAULT_IMAGE_MODEL, apply, inject, name };
