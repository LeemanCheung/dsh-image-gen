...
var OpenAIImageClient = class {
	...
	async generate(request, signal, onProgress) {
		...
		if (reference !== void 0 && this.protocol === "codex-subscription") throw new ImageApiError("Reference-image generation requires API-key mode because the Codex subscription endpoint does not accept image edits.");
...
async function referenceImageFromPath(ctx, exec, filePath) {
	...
	return {
		input: {
			...
			name: stored.ref.name ?? basename(filePath)
		},
...
function apply(ctx, config) {
	...
	const resolveImageAuth = async (signal, needsReference) => {
		if (needsReference && config.authMode === "codex-subscription") throw new Error("reference_image_path requires authMode auto with an API key fallback, or authMode api-key");
		let codexError;
		if (!needsReference && config.authMode !== "api-key") try {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const referenceImagePath = args.reference_image_path === void 0 ? void 0 : args.reference_image_path.trim();
			if (referenceImagePath !== void 0 && (referenceImagePath.length === 0 || referenceImagePath.length > 4096)) throw new Error("reference_image_path must contain 1–4096 characters");
			...
			try {
				const auth = await resolveImageAuth(requestSignal, referenceImagePath !== void 0);
				requestSignal.throwIfAborted();
				const reference = referenceImagePath === void 0 ? void 0 : await referenceImageFromPath(ctx, exec, referenceImagePath);
				requestSignal.throwIfAborted();
				const requestOutputFormat = auth.kind === "codex-subscription" && args.output_format === void 0 ? "png" : outputFormat;
				...
				return {
					...
					...reference === void 0 ? {} : { referenceImage: reference.ref },
					size: generated.size,
