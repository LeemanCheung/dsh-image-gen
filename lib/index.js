...
async function referenceImageFromPath(ctx, exec, filePath) {
	...
	const mediaType$1 = referenceMediaType(filePath);
	if (mediaType$1 === void 0) throw new Error("reference_image_path must name a PNG, JPEG, or WebP image");
	...
	const name$1 = basename(filePath);
...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (requestBackground === "transparent" && outputFormat$1 === "jpeg") throw new Error("transparent backgrounds require PNG or WebP output");
			...
			try {
				const auth = await resolveImageAuth(requestSignal, referenceImagePath !== void 0 ? "reference_image_path" : outputFormat$1 !== "png" ? `${outputFormat$1.toUpperCase()} output` : requestBackground === "transparent" ? "transparent background output" : args.output_compression !== void 0 ? "output_compression" : void 0);
				requestSignal.throwIfAborted();
				const reference = referenceImagePath === void 0 ? void 0 : await referenceImageFromPath(ctx, exec, referenceImagePath);
				requestSignal.throwIfAborted();
				const requestOutputFormat = auth.kind === "codex-subscription" ? "png" : outputFormat$1;
